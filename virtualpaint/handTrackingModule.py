...
class handDetector():
    ...
    def fingerUp(self):
        ...
        # for thumb
        if self.lmList[self.tipid[0]][1]<self.lmList[self.tipid[0]-1][1]:  # thumb is close or not
            fingers.append(1)
        else:
            fingers.append(0)

        for id in range(1,5):
            if self.lmList[self.tipid[id]][2]<self.lmList[self.tipid[id]-2][2]:  # hand is close or not
                fingers.append(1)
            else:
                fingers.append(0)
        return fingers
